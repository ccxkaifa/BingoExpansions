package com.aiyostudio.bingoexpansions.pixelmon.v1_20_2.listen.container;
...
public class EventConsumerContainer {
    ...
    public static final Consumer<BeatWildPixelmonEvent> BEAT_WILD_PIXELMON = (event) -> {
        Player player = Bukkit.getPlayer(event.player.getGameProfile().getId());
    ...
    public static final Consumer<CaptureEvent.SuccessfulCapture> CAPTURE_PIXELMON = (event) -> {
        Player player = Bukkit.getPlayer(event.getPlayer().getGameProfile().getId());
    ...
    public static final Consumer<BeatTrainerEvent> BEAT_TRAINER = (event) -> {
        Player player = Bukkit.getPlayer(event.player.getGameProfile().getId());
        BingoApi.submit(player, "beat_trainer", event.trainer.getName(event.player.getLanguage()), 1);
    };
    /**
     * 钓鱼
     */
    public static final Consumer<FishingEvent.Catch> FISHING_CATCH = (event) -> {
        Player player = Bukkit.getPlayer(event.player.getGameProfile().getId());
        BingoApi.submit(player, "fishing_catch", event.plannedSpawn.getOrCreateEntity().Z().getString(), 1);
    };
    /**
     * 采摘树果
     */
    ...
    public static final Consumer<PixelmonTradeEvent> TRADE_POKEMON = (event) -> {
        Player player1 = Bukkit.getPlayer(event.getPlayer1().getGameProfile().getId()), player2 = Bukkit.getPlayer(event.getPlayer2().getGameProfile().getId());
    ...
    public static final Consumer<EVsGainedEvent> EVS_GAINED = (event) -> {
        if (event.pokemon.getOwnerPlayer() != null) {
            Player player = Bukkit.getPlayer(event.pokemon.getOwnerPlayer().getGameProfile().getId());
    ...
    public static final Consumer<ShopkeeperEvent.Purchase> SHOPKEEPER_PURCHASE = (event) -> {
        Player player = Bukkit.getPlayer(event.getEntityPlayer().getGameProfile().getId());
